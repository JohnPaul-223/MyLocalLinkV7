...
async function adminCreateUser() {
    ...
    if (!name || !email) { showToast("Name and ID required.", "warn"); return; }
    
    // Auto-generate password if empty
    if (!pass) pass = Math.random().toString(36).slice(-8) + "!";
    
    // Ensure email format for Supabase
    if (!email.includes('@')) email += "@locallink.internal";
    
    showToast("Initializing user...", "info");
