...
async function bootApp() {
    if (window.location.protocol === 'file:') {
        showToast("Warning: Running from file:// may cause errors. Use a local server (e.g. Live Server) if possible.", "warn", 6000);
    }
    initMap();
...
function switchTab(tab) {
    ...
    $('nameFieldWrap').classList.toggle('hidden', tab === 'login');
    $('authBtnLabel').textContent = tab === 'login' ? 'Login' : 'Create Account';
}
...
async function handleAuth(e) {
    ...
    try {
        // Attempt login
        const { data, error } = await sb.from('profiles').select('id').eq('email', email).limit(1);
        
        // If profile exists, it's a standard login
        ...
        if (authErr && authErr.message.includes('Invalid login credentials')) {
            ...
            if (count === 0) {
                // System is empty! First-ever login acts as first-ever signup
                const { data: upData, error: upErr } = await sb.auth.signUp({ email, password });
                if (upErr) throw upErr;
                currentUser = upData.user;
                await loadProfile(); // This will handle admin assignment
                hideAuth();
                initRealtime();
                await loadGeofences();
                await loadPeers();
...
async function loadProfile() {
    ...
    try {
        ...
        if (data) {
            ...
            console.log("No profile found, checking system status...");
            const { count } = await sb.from('profiles').select('*', { count: 'exact', head: true });
            
            // If count is 0, make this user the ADMIN
            ...
            if (iErr) {
                ...
                throw new Error("Could not create profile: " + iErr.message);
            }
            userProfile = nData;
            if (role === 'admin') showToast("Initial Setup: You have been granted Admin rights. 🛡️", "success", 6000);
        }
    } catch (err) {
        console.error("Profile load failed:", err);
        showToast("Profile Error: " + (err.message || "Unknown error"), "error", 6000);
    }
    renderHeader();
}
...
async function adminCreateUser() {
    ...
    if (!name || !email || !pass) { showToast("All fields required.", "warn"); return; }
    
    showToast("Creating user...", "info");
    
    // To create a user without disrupting the Admin's session, 
    // we use a temporary client if needed, but standard auth.signUp()
    // often works if 'autoConfirm' is on and we don't 'persistSession'.
    
    const tempSb = supabase.createClient(SUPABASE_URL, SUPABASE_KEY, { auth: { persistSession: false } });
    
    try {
        const { data, error } = await tempSb.auth.signUp({ email, password: pass });
        if (error) throw error;
        
        if (data.user) {
            // Profile will be standard by default via trigger or we insert manually
            await sb.from('profiles').insert({
                id: data.user.id, email, name, role: 'standard', status: 'offline'
            });
