/* ================================================================
   LOCALINK v3 — COMPLETE SCRIPT
   Features: Signup (name + contact + camera photo), Login,
             Realtime Tracking with photo avatars on map,
             Profile popup on marker click (Chat / Call),
             WebRTC Voice/Video Calls, Admin Panel (reset inside),
             SOS, Geofencing
   ================================================================ */

// ─── CONFIGURATION ────────────────────────────────────────────
const SUPABASE_URL = 'https://tmdhcptyssechkrebukc.supabase.co';
const SUPABASE_KEY = 'sb_publishable_IrGGLQ5mp_dk11Hc2bk1mw_ZL1Ya-uq';
const sb           = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);

const TRACK_INTERVAL  = 8000;  // ms between location pushes
const STATIONARY_DIST = 5;     // metres — below this = "stationary"

// ─── STATE ────────────────────────────────────────────────────
let currentUser   = null;
let userProfile   = null;
let isTracking    = false;
let trackTimer    = null;
let lastLatLng    = null;
let map           = null;
let myMarker      = null;
let peerMarkers   = {};   // { userId: L.marker }
let peerProfiles  = {};   // { userId: profileObj }
let geofences     = [];
let geofenceCircles = [];
let insideZones   = {};   // { geofenceId: boolean }
let activeChatUID = null;
let chatSub       = null;
let currentNav    = 'map';

// Camera state
let cameraStream    = null;
let capturedDataURL = null;

// ─── WebRTC ───────────────────────────────────────────────────
let pc           = null;
let localStream  = null;
let callSub      = null;
let activeCallUID = null;
const STUN_CFG   = { iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] };

// ─── HELPERS ─────────────────────────────────────────────────
const $ = id => document.getElementById(id);

function showToast(msg, type = '', duration = 3000) {
    const t = $('toast');
    t.textContent = msg;
    t.className   = `toast show ${type}`;
    setTimeout(() => { t.className = 'toast hidden'; }, duration);
}

function initials(name) {
    if (!name) return '?';
    return name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
}

function fmtTime(ts) {
    return new Date(ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
}

/** Render avatar element: show img if url, otherwise initials */
function renderAvatar(wrapEl, initialsEl, imgEl, name, avatarUrl) {
    if (!wrapEl) return;
    initialsEl.textContent = initials(name);
    if (avatarUrl) {
        imgEl.src = avatarUrl;
        imgEl.style.display = 'block';
        initialsEl.style.display = 'none';
    } else {
        imgEl.style.display = 'none';
        initialsEl.style.display = 'flex';
    }
}

// ─── MAP ─────────────────────────────────────────────────────
function initMap() {
    map = L.map('map', { zoomControl: false }).setView([14.5995, 120.9842], 14);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/light_all/{z}/{x}/{y}.png', {
        attribution: '© OpenStreetMap © CARTO'
    }).addTo(map);
    L.control.zoom({ position: 'bottomright' }).addTo(map);
}

function myIcon() {
    return L.divIcon({
        className: '',
        html: `<div class="my-pin"></div>`,
        iconSize: [20, 20], iconAnchor: [10, 10]
    });
}

function peerIcon(profile) {
    const name   = profile?.name || '?';
    const url    = profile?.avatar_url || '';
    const online = profile?.status === 'online';
    const borderColor = online ? '#22c55e' : '#94a3b8';
    const bgColor     = online ? '#22c55e' : '#94a3b8';
    const letter      = name[0].toUpperCase();

    const inner = url
        ? `<img src="${url}" class="peer-marker-img" onerror="this.style.display='none'">`
        : `<div class="peer-marker-letter" style="background:${bgColor}">${letter}</div>`;

    return L.divIcon({
        className: '',
        html: `<div class="peer-marker-wrap" style="border-color:${borderColor}">${inner}</div>`,
        iconSize: [46, 46], iconAnchor: [23, 23]
    });
}

function updateMyMarker(lat, lng) {
    if (!myMarker) {
        myMarker = L.marker([lat, lng], { icon: myIcon(), zIndexOffset: 1000 }).addTo(map)
            .bindPopup('<b>You</b>');
    } else {
        myMarker.setLatLng([lat, lng]);
    }
}

function updatePeerMarker(uid, lat, lng) {
    const prof = peerProfiles[uid];
    if (!peerMarkers[uid]) {
        peerMarkers[uid] = L.marker([lat, lng], { icon: peerIcon(prof) })
            .addTo(map)
            .on('click', () => openProfile(uid));
    } else {
        peerMarkers[uid].setLatLng([lat, lng]);
        peerMarkers[uid].setIcon(peerIcon(prof));
    }
}

// ─── AUTH SCREEN ─────────────────────────────────────────────
function switchAuthTab(tab) {
    if (tab === 'login') {
        $('loginTab').classList.add('active');
        $('signupTab').classList.remove('active');
        $('loginForm').classList.remove('hidden');
        $('signupForm').classList.add('hidden');
        $('authSubtitle').textContent = 'Welcome back!';
        stopCamera();
    } else {
        $('signupTab').classList.add('active');
        $('loginTab').classList.remove('active');
        $('signupForm').classList.remove('hidden');
        $('loginForm').classList.add('hidden');
        $('authSubtitle').textContent = 'Create your account';
    }
}

function showAuth() {
    $('authOverlay').classList.remove('hidden');
}
function hideAuth() {
    $('authOverlay').classList.add('hidden');
    stopCamera();
}

// ─── PUSH NOTIFICATIONS ───────────────────────────────────────
async function registerServiceWorker() {
    if (!('serviceWorker' in navigator)) {
        console.warn('Service Workers not supported');
        return;
    }
    try {
        const reg = await navigator.serviceWorker.register('sw.js', { scope: '/' });
        console.log('✅ Service Worker registered for push notifications');
    } catch (err) {
        console.error('Service Worker registration failed:', err);
    }
}

async function requestNotificationPermission() {
    if (!('Notification' in window)) {
        console.warn('Notifications not supported');
        return false;
    }
    
    if (Notification.permission === 'granted') {
        return true;
    }
    
    if (Notification.permission !== 'denied') {
        const permission = await Notification.requestPermission();
        return permission === 'granted';
    }
    return false;
}

function showLocalNotification(title, options = {}) {
    if (!('Notification' in window)) return;
    if (Notification.permission !== 'granted') return;
    
    const notification = new Notification(title, {
        icon: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192"><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="120" fill="%235b6af5">📍</text></svg>',
        badge: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192"><circle cx="96" cy="96" r="90" fill="%235b6af5"/></svg>',
        ...options
    });
    
    notification.onclick = () => {
        window.focus();
        notification.close();
    };
}

// ─── CAMERA ──────────────────────────────────────────────────
async function openCamera() {
    try {
        cameraStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false });
        const video  = $('cameraStream');
        video.srcObject = cameraStream;
        video.classList.remove('hidden');
        $('cameraPlaceholder').classList.add('hidden');
        $('capturedPhoto').classList.add('hidden');
        $('openCamBtn').classList.add('hidden');
        $('snapBtn').classList.remove('hidden');
        $('retakeBtn').classList.add('hidden');
        capturedDataURL = null;
    } catch (err) {
        showToast('Camera access denied. Please allow camera.', 'error');
    }
}

function capturePhoto() {
    const video  = $('cameraStream');
    const canvas = $('cameraCanvas');
    canvas.width  = video.videoWidth  || 640;
    canvas.height = video.videoHeight || 480;
    const ctx = canvas.getContext('2d');
    // mirror-flip to match natural selfie orientation
    ctx.translate(canvas.width, 0);
    ctx.scale(-1, 1);
    ctx.drawImage(video, 0, 0);
    capturedDataURL = canvas.toDataURL('image/jpeg', 0.8);

    $('capturedPhoto').src = capturedDataURL;
    $('capturedPhoto').classList.remove('hidden');
    video.classList.add('hidden');
    $('snapBtn').classList.add('hidden');
    $('retakeBtn').classList.remove('hidden');
    stopCamera();
}

function retakePhoto() {
    capturedDataURL = null;
    $('capturedPhoto').classList.add('hidden');
    $('retakeBtn').classList.add('hidden');
    $('cameraStream').classList.add('hidden');
    $('cameraPlaceholder').classList.remove('hidden');
    $('openCamBtn').classList.remove('hidden');
}

function stopCamera() {
    if (cameraStream) {
        cameraStream.getTracks().forEach(t => t.stop());
        cameraStream = null;
    }
}

// ─── LOGIN ────────────────────────────────────────────────────
async function handleLogin(e) {
    e.preventDefault();
    const loginId  = $('loginId').value.trim();
    const password = $('loginPass').value;
    $('loginBtnLabel').textContent = 'Verifying…';

    try {
        const { data: user, error } = await sb.from('profiles')
            .select('*')
            .eq('login_id', loginId)
            .eq('password', password)
            .single();

        if (error || !user) throw new Error('Invalid Login ID or Password.');

        userProfile = user;
        currentUser = { id: user.id, email: user.login_id };
        localStorage.setItem('localink_user', JSON.stringify({ id: user.id, pass: user.password }));
        onLoginSuccess();
    } catch (err) {
        showToast(err.message, 'error');
    }
    $('loginBtnLabel').textContent = 'Login';
}

// ─── SIGNUP ───────────────────────────────────────────────────
async function handleSignup(e) {
    e.preventDefault();
    const firstName = $('signupFirst').value.trim();
    const lastName  = $('signupLast').value.trim();
    const contact   = $('signupContact').value.trim();
    const loginId   = $('signupLoginId').value.trim();
    const password  = $('signupPass').value;

    if (!firstName || !lastName) { showToast('First and last name required.', 'error'); return; }
    if (!contact)                { showToast('Contact number required.', 'error'); return; }
    if (!loginId || !password)   { showToast('Login ID and password required.', 'error'); return; }

    $('signupBtnLabel').textContent = 'Creating…';

    try {
        // Check if login_id already taken
        const { data: existing } = await sb.from('profiles').select('id').eq('login_id', loginId).single();
        if (existing) throw new Error('Login ID already taken. Choose another.');

        // Determine role — first ever user becomes admin
        const { count } = await sb.from('profiles').select('*', { count: 'exact', head: true });
        const role = (count === 0) ? 'admin' : 'standard';

        // Upload photo if captured
        let avatarUrl = null;
        if (capturedDataURL) {
            avatarUrl = await uploadProfilePhoto(capturedDataURL, loginId);
        }

        const fullName = `${firstName} ${lastName}`;
        const { data: newUser, error: insertErr } = await sb.from('profiles').insert({
            name:       fullName,
            login_id:   loginId,
            password:   password,
            contact:    contact,
            role:       role,
            avatar_url: avatarUrl,
            status:     'offline'
        }).select().single();

        if (insertErr) throw insertErr;

        userProfile = newUser;
        currentUser = { id: newUser.id, email: newUser.login_id };
        localStorage.setItem('localink_user', JSON.stringify({ id: newUser.id, pass: newUser.password }));
        showToast(`Welcome, ${firstName}! 🎉`, 'success');
        onLoginSuccess();
    } catch (err) {
        showToast(err.message, 'error');
    }
    $('signupBtnLabel').textContent = 'Create Account';
}

// ─── PHOTO UPLOAD ─────────────────────────────────────────────
async function uploadProfilePhoto(dataUrl, loginId) {
    try {
        // Convert dataURL -> blob
        const res  = await fetch(dataUrl);
        const blob = await res.blob();
        const filename = `profiles/${loginId.replace(/[^a-z0-9]/gi,'_')}_${Date.now()}.jpg`;

        const { data, error } = await sb.storage
            .from('avatars')
            .upload(filename, blob, { contentType: 'image/jpeg', upsert: true });

        if (error) { console.warn('Photo upload failed:', error.message); return null; }

        const { data: urlData } = sb.storage.from('avatars').getPublicUrl(filename);
        return urlData?.publicUrl || null;
    } catch (err) {
        console.warn('Photo upload error:', err);
        return null;
    }
}

// ─── BOOT ────────────────────────────────────────────────────
async function bootApp() {
    initMap();
    const saved = localStorage.getItem('localink_user');
    if (saved) {
        try {
            const parsed = JSON.parse(saved);
            const { data } = await sb.from('profiles').select('*').eq('id', parsed.id).eq('password', parsed.pass).single();
            if (data) {
                userProfile = data;
                currentUser = { id: data.id, email: data.login_id };
                onLoginSuccess();
                return;
            }
        } catch(e) {}
    }
    showAuth();
}

async function onLoginSuccess() {
    renderHeader();
    hideAuth();
    await registerServiceWorker();
    await requestNotificationPermission();
    initRealtime();
    await loadGeofences();
    await loadPeers();
    renderContactList(currentNav);
    switchNav('map');
}

function renderHeader() {
    if (!userProfile) return;
    $('displayName').textContent = userProfile.name || 'User';
    renderAvatar(
        $('avatarWrap'),
        $('avatarEl'),
        $('avatarImg'),
        userProfile.name,
        userProfile.avatar_url
    );
}

async function logout() {
    localStorage.removeItem('localink_user');
    await stopTracking();
    location.reload();
}

// ─── NAVIGATION ───────────────────────────────────────────────
function switchNav(tab) {
    closeMenu();
    currentNav = tab;
    document.querySelectorAll('.nav-item').forEach(el => el.classList.remove('active'));
    const navEl = $(`nav${tab.charAt(0).toUpperCase() + tab.slice(1)}`);
    if (navEl) navEl.classList.add('active');

    if (tab === 'map') {
        $('sidePanel').classList.add('hidden');
        $('bottomCard').classList.remove('hidden');
        if (window.innerWidth > 520) $('sidePanel').classList.remove('hidden');
    } else if (tab === 'chat' || tab === 'call') {
        $('sidePanel').classList.remove('hidden');
        $('bottomCard').classList.add('hidden');
        $('sidePanel').querySelector('.panel-title').textContent = tab === 'chat' ? 'Select to Chat' : 'Select to Call';
        renderContactList(tab);
    } else if (tab === 'admin') {
        openAdmin();
    } else if (tab === 'about') {
        openAbout();
    }
}

function openAbout()  { $('aboutOverlay').classList.remove('hidden'); }
function closeAbout() { $('aboutOverlay').classList.add('hidden'); switchNav('map'); }

function openMenu()   { $('menuOverlay').classList.remove('hidden'); }
function closeMenu()  { $('menuOverlay').classList.add('hidden'); }

// ─── REALTIME ────────────────────────────────────────────────
function initRealtime() {
    sb.channel('locations-channel')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'locations' }, async (payload) => {
            const loc = payload.new;
            if (loc.user_id === currentUser.id) return;
            if (!peerProfiles[loc.user_id]) await loadPeers();
            // Only show marker if user is online
            const prof = peerProfiles[loc.user_id];
            if (prof && prof.status === 'online') {
                updatePeerMarker(loc.user_id, loc.lat, loc.lng);
            }
        })
        .subscribe();

    sb.channel('profiles-channel')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, (payload) => {
            const p = payload.new;
            if (p.id === currentUser.id) return;
            peerProfiles[p.id] = p;
            renderContactList(currentNav);
            if (p.status === 'online') {
                // Re-draw marker with updated icon (photo/name may have changed)
                const latlng = peerMarkers[p.id]?.getLatLng();
                if (latlng) updatePeerMarker(p.id, latlng.lat, latlng.lng);
            } else {
                // Remove marker when user goes offline
                if (peerMarkers[p.id]) {
                    peerMarkers[p.id].remove();
                    delete peerMarkers[p.id];
                }
            }
        })
        .subscribe();

    sb.channel('messages-channel')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async (payload) => {
            const msg = payload.new;
            
            // Check if this message is for current user
            const isDirectMessage = msg.receiver_id === currentUser.id;
            const isSOSAlert = msg.receiver_id === null && msg.sender_id !== currentUser.id;
            
            if (!isDirectMessage && !isSOSAlert) return;
            
            // Load sender profile if needed
            if (!peerProfiles[msg.sender_id]) await loadPeers();
            const sender = peerProfiles[msg.sender_id];
            
            // Handle SOS alerts (broadcast messages)
            if (isSOSAlert) {
                showLocalNotification(`🆘 SOS ALERT from ${sender?.name || 'Someone'}!`, {
                    body: msg.content.substring(0, 80),
                    tag: 'sos-alert',
                    requireInteraction: true
                });
                showToast(`🆘 ${sender?.name} sent an SOS alert!`, 'warn', 5000);
                return;
            }
            
            // Handle regular chat messages
            showLocalNotification(`💬 ${sender?.name || 'User'}`, {
                body: msg.content.substring(0, 60),
                tag: `chat-${msg.sender_id}`,
                requireInteraction: false
            });
            showToast(`💬 Message from ${sender?.name || 'User'}`, 'success', 3000);
        })
        .subscribe();

    callSub = sb.channel('call-signal')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'call_signals' }, async (p) => {
            const sig = p.new;
            if (sig.callee_id !== currentUser.id) return;
            
            // Load caller profile for notification
            if (!peerProfiles[sig.caller_id]) await loadPeers();
            const caller = peerProfiles[sig.caller_id];
            
            if (sig.type === 'offer') {
                // Show incoming call notification immediately
                showLocalNotification(`📞 ${caller?.name || 'Someone'} is calling...`, {
                    body: sig.data.withVideo ? '📹 Video Call' : '🎙️ Voice Call',
                    tag: 'incoming-call',
                    requireInteraction: true
                });
                handleIncomingCall(sig);
            }
            if (sig.type === 'answer') handleCallAnswer(sig);
            if (sig.type === 'ice')    handleRemoteICE(sig);
            if (sig.type === 'end')    endCall(false);
        })
        .subscribe();
}

// ─── TRACKING ────────────────────────────────────────────────
async function toggleTracking() {
    if (isTracking) await stopTracking();
    else            await startTracking();
}

async function startTracking() {
    if (!navigator.geolocation) { showToast('GPS not available.', 'error'); return; }
    isTracking = true;
    updateTrackingUI(true);
    sendLocation();
    trackTimer = setInterval(sendLocation, TRACK_INTERVAL);
}

async function stopTracking() {
    if (trackTimer) { clearInterval(trackTimer); trackTimer = null; }
    if (myMarker)   { myMarker.remove(); myMarker = null; }
    isTracking = false;
    updateTrackingUI(false);
    if (currentUser) {
        await sb.from('profiles').update({ status: 'offline' }).eq('id', currentUser.id);
    }
}

function sendLocation() {
    navigator.geolocation.getCurrentPosition(async (pos) => {
        const { latitude: lat, longitude: lng } = pos.coords;
        const newLatLng = L.latLng(lat, lng);

        if (lastLatLng && lastLatLng.distanceTo(newLatLng) < STATIONARY_DIST) {
            $('locationCaption').textContent = 'Stationary – saving battery';
            return;
        }
        lastLatLng = newLatLng;
        $('locationCaption').textContent = `Lat: ${lat.toFixed(5)}, Lng: ${lng.toFixed(5)}`;

        updateMyMarker(lat, lng);
        map.panTo([lat, lng]);

        await sb.from('locations').insert({ user_id: currentUser.id, lat, lng, timestamp: new Date().toISOString() });
        await sb.from('profiles').update({ status: 'online' }).eq('id', currentUser.id);
        checkGeofences(lat, lng);
    }, null, { enableHighAccuracy: true, timeout: 8000 });
}

function updateTrackingUI(online) {
    const btn = $('toggleBtn');
    const dot = $('statusLabel');
    if (online) {
        btn.textContent = 'Stop Sharing';
        btn.className   = 'toggle-btn offline-btn';
        dot.innerHTML   = '<span class="dot online"></span> Online & Sharing';
    } else {
        btn.textContent = 'Go Online';
        btn.className   = 'toggle-btn online-btn';
        dot.innerHTML   = '<span class="dot offline"></span> Offline';
        $('locationCaption').textContent = 'Press Go Online to start';
    }
}

// ─── GEOFENCING ──────────────────────────────────────────────
async function loadGeofences() {
    const { data } = await sb.from('geofences').select('*');
    if (!data) return;
    geofences = data;
    geofenceCircles.forEach(c => c.remove());
    geofenceCircles = [];
    geofences.forEach(gf => {
        const circle = L.circle([gf.lat, gf.lng], {
            radius: gf.radius, color: '#5b6af5',
            fillColor: '#5b6af5', fillOpacity: 0.07, weight: 1.5
        }).addTo(map).bindTooltip(gf.name, { permanent: true, direction: 'center', className: 'gf-label' });
        geofenceCircles.push(circle);
        insideZones[gf.id] = false;
    });
}

async function checkGeofences(lat, lng) {
    for (const gf of geofences) {
        const dist   = L.latLng(lat, lng).distanceTo(L.latLng(gf.lat, gf.lng));
        const inside = dist <= gf.radius;
        const wasIn  = insideZones[gf.id];
        if (inside && !wasIn) {
            insideZones[gf.id] = true;
            showToast(`📍 You arrived at ${gf.name}`, 'success');
            await sb.from('geofence_logs').insert({ user_id: currentUser.id, geofence_id: gf.id, event: 'enter' });
        } else if (!inside && wasIn) {
            insideZones[gf.id] = false;
            showToast(`👋 You left ${gf.name}`, 'warn');
            await sb.from('geofence_logs').insert({ user_id: currentUser.id, geofence_id: gf.id, event: 'exit' });
        }
    }
}

// ─── CONTACTS & PEERS ────────────────────────────────────────
async function loadPeers() {
    const { data } = await sb.from('profiles').select('*').neq('id', currentUser.id);
    if (data) data.forEach(p => { peerProfiles[p.id] = p; });
    // Only pin ONLINE peers on the map
    for (const uid of Object.keys(peerProfiles)) {
        const prof = peerProfiles[uid];
        if (prof.status !== 'online') continue;
        const { data: loc } = await sb.from('locations')
            .select('lat,lng').eq('user_id', uid).order('timestamp', { ascending: false }).limit(1);
        if (loc && loc[0]) updatePeerMarker(uid, loc[0].lat, loc[0].lng);
    }
}

function renderContactList(mode = 'chat') {
    const list = $('contactsList');
    const uids = Object.keys(peerProfiles);
    if (uids.length === 0) {
        list.innerHTML = '<p class="muted">No contacts yet.</p>';
        return;
    }
    list.innerHTML = uids.map(uid => {
        const p      = peerProfiles[uid];
        const online = p.status === 'online';
        const action = mode === 'chat' ? `openChat('${uid}')` : `startCall('${uid}', false)`;
        const avatarHtml = p.avatar_url
            ? `<div class="avatar-wrap sm" style="position:relative;"><img class="avatar-img" src="${p.avatar_url}" style="display:block;position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:50%"><div class="avatar-initials sm" style="display:none">${initials(p.name)}</div></div>`
            : `<div class="avatar-wrap sm"><div class="avatar-initials sm">${initials(p.name)}</div></div>`;
        return `<div class="contact-item" onclick="${action}">
            ${avatarHtml}
            <div>
                <div class="username">${p.name || 'User'}</div>
                <div class="muted small">${online ? '🟢 Online' : '⚫ Offline'}</div>
            </div>
        </div>`;
    }).join('');
}

// ─── PROFILE POPUP (map marker click) ────────────────────────
function openProfile(uid) {
    const p = peerProfiles[uid];
    if (!p) return;

    // Avatar
    const ppAvatarImg  = $('ppAvatarImg');
    const ppAvatarInit = $('ppAvatar');
    ppAvatarInit.textContent = initials(p.name);
    if (p.avatar_url) {
        ppAvatarImg.src = p.avatar_url;
        ppAvatarImg.classList.remove('hidden');
        ppAvatarInit.style.display = 'none';
    } else {
        ppAvatarImg.classList.add('hidden');
        ppAvatarInit.style.display = 'flex';
    }

    $('ppName').textContent    = p.name || 'User';
    $('ppContact').textContent = p.contact ? `📞 ${p.contact}` : '📵 No contact';
    $('ppStatus').textContent  = p.status === 'online' ? '🟢 Online & Sharing Location' : '⚫ Offline';

    // Wire buttons — always fresh closures so correct uid is captured
    $('ppChatBtn').onclick  = () => { closeProfile(); openChat(uid); };
    $('ppCallBtn').onclick  = () => { closeProfile(); startCall(uid, false); };
    $('ppVideoBtn').onclick = () => { closeProfile(); startCall(uid, true); };

    const popup = $('profilePopup');
    popup.classList.remove('hidden');
    // Re-trigger animation on re-open
    popup.style.animation = 'none';
    popup.offsetHeight; // reflow
    popup.style.animation = '';
}

function closeProfile() { $('profilePopup').classList.add('hidden'); }

// ─── CHAT ────────────────────────────────────────────────────
async function openChat(uid) {
    activeChatUID = uid;
    const p = peerProfiles[uid];

    $('chatTitle').textContent  = p?.name || 'Chat';
    $('chatStatus').textContent = p?.status === 'online' ? '🟢 Online' : '⚫ Offline';
    renderAvatar(
        $('chatAvatarWrap'),
        $('chatAvatar'),
        $('chatAvatarImg'),
        p?.name,
        p?.avatar_url
    );
    $('chatWindow').classList.remove('hidden');
    await fetchChatHistory();

    if (chatSub) sb.removeChannel(chatSub);
    chatSub = sb.channel('chat-' + [currentUser.id, uid].sort().join('-'))
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (pl) => {
            const m = pl.new;
            if ((m.sender_id === currentUser.id && m.receiver_id === activeChatUID) ||
                (m.sender_id === activeChatUID   && m.receiver_id === currentUser.id)) {
                appendBubble(m);
            }
        })
        .subscribe();
}

function closeChat() {
    activeChatUID = null;
    if (chatSub) { sb.removeChannel(chatSub); chatSub = null; }
    $('chatWindow').classList.add('hidden');
}

async function fetchChatHistory() {
    const uid = activeChatUID;
    const { data } = await sb.from('messages')
        .select('*')
        .or(`and(sender_id.eq.${currentUser.id},receiver_id.eq.${uid}),and(sender_id.eq.${uid},receiver_id.eq.${currentUser.id})`)
        .order('timestamp', { ascending: true })
        .limit(80);
    const body = $('chatBody');
    body.innerHTML = '';
    if (!data || data.length === 0) {
        body.innerHTML = '<p class="muted center-text">Start the conversation 👋</p>';
        return;
    }
    data.forEach(m => appendBubble(m, false));
    body.scrollTop = body.scrollHeight;
}

function appendBubble(m, scroll = true) {
    const sent = m.sender_id === currentUser.id;
    const body = $('chatBody');
    const div  = document.createElement('div');
    div.style.display       = 'flex';
    div.style.flexDirection = 'column';
    div.style.alignItems    = sent ? 'flex-end' : 'flex-start';
    div.innerHTML = `
        <div class="bubble ${sent ? 'sent' : 'received'}">${escHTML(m.content)}</div>
        <div class="bubble-time">${fmtTime(m.timestamp)}</div>`;
    body.appendChild(div);
    if (scroll) body.scrollTop = body.scrollHeight;
}

function escHTML(str) {
    return str.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
}

async function sendMessage() {
    if (!activeChatUID) return;
    const input   = $('msgInput');
    const content = input.value.trim();
    if (!content) return;
    input.value = '';
    
    const senderName = userProfile?.name || 'User';
    await sb.from('messages').insert({
        sender_id: currentUser.id, receiver_id: activeChatUID,
        content, timestamp: new Date().toISOString()
    });
    
    // Show local notification
    showLocalNotification(`💬 Message sent to ${peerProfiles[activeChatUID]?.name || 'User'}`, {
        body: content.substring(0, 50),
        tag: 'chat',
        requireInteraction: false
    });
}

// ─── WebRTC CALLS ────────────────────────────────────────────
async function startCall(uid, withVideo = false) {
    activeCallUID = uid;
    const p = peerProfiles[uid];
    showCallUI(p?.name || 'User', 'Calling…', p?.avatar_url);

    localStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: withVideo });
    $('localVideo').srcObject = localStream;
    if (!withVideo) $('localVideo').style.display = 'none';

    pc = new RTCPeerConnection(STUN_CFG);
    localStream.getTracks().forEach(t => pc.addTrack(t, localStream));
    pc.ontrack = e => { $('remoteVideo').srcObject = e.streams[0]; };
    pc.onicecandidate = e => { if (e.candidate) pushSignal(uid, 'ice', { candidate: e.candidate }); };

    const offer = await pc.createOffer();
    await pc.setLocalDescription(offer);
    await pushSignal(uid, 'offer', { sdp: offer, withVideo });
    $('callState').textContent = 'Ringing…';
    
    // Show notification for call initiated
    showLocalNotification(`📞 ${withVideo ? 'Video' : 'Voice'} call to ${p?.name || 'User'}`, {
        body: 'Waiting for response...',
        tag: 'call',
        requireInteraction: true
    });
}

async function handleIncomingCall(sig) {
    const caller = peerProfiles[sig.caller_id];
    
    // Show notification
    showLocalNotification(`📞 Incoming ${sig.data.withVideo ? 'video' : 'voice'} call`, {
        body: `From ${caller?.name || 'Someone'}`,
        tag: 'incoming-call',
        requireInteraction: true
    });
    
    if (!confirm(`📞 Incoming call from ${caller?.name || 'Someone'}. Accept?`)) {
        await pushSignal(sig.caller_id, 'end', {});
        return;
    }
    activeCallUID = sig.caller_id;
    showCallUI(caller?.name || 'User', 'Connected', caller?.avatar_url);

    localStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: sig.data.withVideo });
    $('localVideo').srcObject = localStream;
    if (!sig.data.withVideo) $('localVideo').style.display = 'none';

    pc = new RTCPeerConnection(STUN_CFG);
    localStream.getTracks().forEach(t => pc.addTrack(t, localStream));
    pc.ontrack = e => { $('remoteVideo').srcObject = e.streams[0]; };
    pc.onicecandidate = e => { if (e.candidate) pushSignal(sig.caller_id, 'ice', { candidate: e.candidate }); };

    await pc.setRemoteDescription(new RTCSessionDescription(sig.data.sdp));
    const answer = await pc.createAnswer();
    await pc.setLocalDescription(answer);
    await pushSignal(sig.caller_id, 'answer', { sdp: answer });
}

async function handleCallAnswer(sig) {
    if (!pc) return;
    await pc.setRemoteDescription(new RTCSessionDescription(sig.data.sdp));
    $('callState').textContent = 'Connected';
}

async function handleRemoteICE(sig) {
    if (!pc || !sig.data.candidate) return;
    try { await pc.addIceCandidate(new RTCIceCandidate(sig.data.candidate)); } catch(e) {}
}

async function endCall(sendSignal = true) {
    if (sendSignal && activeCallUID) await pushSignal(activeCallUID, 'end', {});
    if (pc) { pc.close(); pc = null; }
    if (localStream) { localStream.getTracks().forEach(t => t.stop()); localStream = null; }
    $('callOverlay').classList.add('hidden');
    $('localVideo').srcObject  = null;
    $('remoteVideo').srcObject = null;
    activeCallUID = null;
}

async function pushSignal(calleeId, type, data) {
    await sb.from('call_signals').insert({ caller_id: currentUser.id, callee_id: calleeId, type, data });
}

function showCallUI(name, state, avatarUrl) {
    $('callName').textContent  = name;
    $('callState').textContent = state;
    renderAvatar(
        $('callAvatarWrap'),
        $('callAvatar'),
        $('callAvatarImg'),
        name,
        avatarUrl
    );
    $('callOverlay').classList.remove('hidden');
}

function toggleMic() {
    if (!localStream) return;
    const track = localStream.getAudioTracks()[0];
    if (track) { track.enabled = !track.enabled; showToast(track.enabled ? '🎙️ Mic on' : '🔇 Mic off'); }
}

function toggleCamera() {
    if (!localStream) return;
    const track = localStream.getVideoTracks()[0];
    if (track) { track.enabled = !track.enabled; showToast(track.enabled ? '📷 Camera on' : '📷 Camera off'); }
}

// ─── SOS ────────────────────────────────────────────────────
async function sendSOS() {
    if (!confirm('🚨 Send SOS alert with your location to all contacts?')) return;
    navigator.geolocation.getCurrentPosition(async (pos) => {
        const { latitude: lat, longitude: lng } = pos.coords;
        await sb.from('messages').insert({
            sender_id: currentUser.id, receiver_id: null,
            content: `🆘 SOS ALERT from ${userProfile?.name}! Location: ${lat.toFixed(5)}, ${lng.toFixed(5)} — https://maps.google.com/?q=${lat},${lng}`,
            timestamp: new Date().toISOString()
        });
        showToast('🆘 SOS alert sent!', 'error', 5000);
    });
}

// ─── ADMIN PANEL ─────────────────────────────────────────────
function openAdmin() {
    $('adminOverlay').classList.remove('hidden');
    const isAdmin = userProfile?.role === 'admin';

    const tabs = $('adminTabs');
    if (tabs) tabs.classList.toggle('hidden', !isAdmin);

    ['adminUsers','adminGeofences','adminGroups','adminSystem'].forEach(id => {
        const el = $(id); if (el) el.classList.add('hidden');
    });

    const lock = $('adminAuthLock');
    if (lock) lock.classList.toggle('hidden', isAdmin);

    if (isAdmin) {
        $('adminUsers').classList.remove('hidden');
        loadAdminUsers();
    }
}
function closeAdmin() { $('adminOverlay').classList.add('hidden'); }

function adminTab(tab, btn) {
    ['adminUsers','adminGeofences','adminGroups','adminSystem'].forEach(id => {
        const el = $(id); if (el) el.classList.add('hidden');
    });
    const map = { users:'adminUsers', geofences:'adminGeofences', groups:'adminGroups', system:'adminSystem' };
    if ($(map[tab])) $(map[tab]).classList.remove('hidden');

    document.querySelectorAll('.tab-row .tab').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');

    if (tab === 'users')     loadAdminUsers();
    if (tab === 'geofences') loadAdminGeofences();
    if (tab === 'groups')    loadAdminGroups();
}

async function loadAdminUsers() {
    const { data } = await sb.from('profiles').select('*');
    const tbody = $('usersTableBody');
    if (!data || data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="muted">No users.</td></tr>';
        return;
    }
    tbody.innerHTML = data.map(u => `
        <tr>
            <td>${u.name || '—'}</td>
            <td>${u.login_id || '—'}</td>
            <td>
                <select onchange="setRole('${u.id}', this.value)" style="border:none;background:transparent;font-family:Outfit,sans-serif;cursor:pointer;">
                    <option value="standard" ${u.role==='standard'?'selected':''}>Standard</option>
                    <option value="admin"    ${u.role==='admin'?'selected':''}>Admin</option>
                </select>
            </td>
            <td><button class="btn danger" style="padding:4px 10px;font-size:0.75rem;" onclick="deleteUser('${u.id}')">Delete</button></td>
        </tr>`).join('');
}

async function adminCreateUser() {
    const name    = $('newUserName').value.trim();
    const loginId = $('newUserEmail').value.trim();
    let   pass    = $('newUserPass').value.trim();

    if (!name || !loginId) { showToast('Name and ID required.', 'warn'); return; }
    if (!pass) pass = Math.random().toString(36).slice(-8);

    showToast('Saving user…', 'info');
    try {
        const { data, error } = await sb.from('profiles').insert({
            name, login_id: loginId, password: pass, role: 'standard'
        }).select().single();
        if (error) {
            if (error.message.includes('unique')) throw new Error('Login ID already taken.');
            throw error;
        }
        alert(`✅ User Created!\n\nName: ${name}\nLogin ID: ${loginId}\nPassword: ${pass}`);
        $('newUserName').value = ''; $('newUserEmail').value = ''; $('newUserPass').value = '';
        $('createUserForm').classList.add('hidden');
        loadAdminUsers();
    } catch(err) {
        showToast(err.message, 'error');
    }
}

async function setRole(uid, role) {
    await sb.from('profiles').update({ role }).eq('id', uid);
    showToast('Role updated ✓', 'success');
}

async function deleteUser(uid) {
    if (!confirm('Delete this user?')) return;
    await sb.from('profiles').delete().eq('id', uid);
    showToast('User deleted.', 'warn');
    loadAdminUsers();
    delete peerProfiles[uid];
    renderContactList();
}

async function loadAdminGeofences() {
    const { data } = await sb.from('geofences').select('*');
    const list = $('geofencesList');
    if (!data || data.length === 0) { list.innerHTML = '<p class="muted">No zones yet.</p>'; return; }
    list.innerHTML = data.map(gf => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #f1f5f9;">
            <span><b>${gf.name}</b> — ${gf.lat.toFixed(4)}, ${gf.lng.toFixed(4)} (${gf.radius}m)</span>
            <button class="btn danger" style="padding:4px 10px;font-size:0.75rem;" onclick="deleteGeofence('${gf.id}')">×</button>
        </div>`).join('');
}

async function addGeofence() {
    const name = $('gfName').value.trim(), lat = parseFloat($('gfLat').value),
          lng  = parseFloat($('gfLng').value), radius = parseFloat($('gfRadius').value);
    if (!name || isNaN(lat) || isNaN(lng) || isNaN(radius)) { showToast('Fill all zone fields.', 'error'); return; }
    await sb.from('geofences').insert({ name, lat, lng, radius, created_by: currentUser.id });
    showToast(`Zone "${name}" added ✓`, 'success');
    [$('gfName'),$('gfLat'),$('gfLng'),$('gfRadius')].forEach(f => f.value = '');
    loadAdminGeofences();
    await loadGeofences();
}

async function deleteGeofence(id) {
    await sb.from('geofences').delete().eq('id', id);
    showToast('Zone removed.', 'warn');
    loadAdminGeofences();
    await loadGeofences();
}

async function loadAdminGroups() {
    const { data } = await sb.from('groups').select('*');
    const list = $('groupsList');
    if (!data || data.length === 0) { list.innerHTML = '<p class="muted">No groups yet.</p>'; return; }
    list.innerHTML = data.map(g => `
        <div style="padding:6px 0;border-bottom:1px solid #f1f5f9;"><b>${g.name}</b></div>`).join('');
}

async function createGroup() {
    const name = $('grpName').value.trim();
    if (!name) return;
    await sb.from('groups').insert({ name, created_by: currentUser.id });
    $('grpName').value = '';
    showToast(`Group "${name}" created ✓`, 'success');
    loadAdminGroups();
}

// ─── NUCLEAR RESET (Admin Panel only) ─────────────────────────
async function nuclearReset() {
    if (!confirm('🚨 TOTAL SYSTEM WIPE: This will delete ALL users, tracking data, and messages. This cannot be undone. Proceed?')) return;
    showToast('Wiping database…', 'warn');
    try {
        const tables = ['locations','messages','geofences','geofence_logs','call_signals','profiles'];
        for (const table of tables) {
            const { error } = await sb.from(table).delete().neq('id', '00000000-0000-0000-0000-000000000000');
            if (error) console.warn(`Note: Could not clear ${table}:`, error.message);
        }
        showToast('Database Wipe Successful! Reloading…', 'success', 5000);
        localStorage.removeItem('localink_user');
        setTimeout(() => location.reload(), 2500);
    } catch(err) {
        showToast('Error during wipe: ' + err.message, 'error');
    }
}

// ─── EVENT WIRING ────────────────────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
    bootApp();

    $('toggleBtn').addEventListener('click',   toggleTracking);
    $('logoutBtn').addEventListener('click',   logout);
    $('menuBtn').addEventListener('click',     openMenu);
    $('sosBtn').addEventListener('click',      sendSOS);

    $('voiceCallBtn').addEventListener('click', () => activeChatUID && startCall(activeChatUID, false));
    $('videoCallBtn').addEventListener('click', () => activeChatUID && startCall(activeChatUID, true));
    $('muteMicBtn').addEventListener('click',   toggleMic);
    $('toggleCamBtn').addEventListener('click', toggleCamera);
    $('endCallBtn').addEventListener('click',   () => endCall(true));

    $('sendBtn').addEventListener('click', sendMessage);
    $('msgInput').addEventListener('keydown', e => { if (e.key === 'Enter') sendMessage(); });

    // Click outside profile popup to close
    document.addEventListener('click', e => {
        const popup = $('profilePopup');
        if (!popup.classList.contains('hidden') && !popup.contains(e.target)) {
            // Only close if not clicking a map marker
            const target = e.target;
            if (!target.closest('.peer-marker-wrap') && !target.closest('.leaflet-marker-icon')) {
                closeProfile();
            }
        }
    });

    // Map click: capture geofence coords in admin
    map.on('click', (e) => {
        if (!$('adminOverlay').classList.contains('hidden')) {
            $('gfLat').value = e.latlng.lat.toFixed(6);
            $('gfLng').value = e.latlng.lng.toFixed(6);
            showToast('📍 Coordinates captured from map click!', 'success');
        }
    });
});
